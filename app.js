...
function viewTitle(v){
  return ({inicio:'Inicio',calendario:'Calendario producción',calendario_publicacion:'Calendario publicación',kanban:'Kanban',ideas:'Ideas',biblioteca:'Biblioteca',clientes:'Posibles clientes',metricas:'Métricas',config:'Configuración'})[v] || 'Content Hub';
}
...
function optionValues(kind){
  const fromDb = state.content_options
    ...
    .map(o=>o.label);
  const base = kind==='channel' ? DEFAULT_CHANNELS : DEFAULT_CONTENT_TYPES;
...
function render(){
  ...
  const views = { inicio: renderInicio, calendario: renderCalendario, calendario_publicacion: renderCalendarioPublicacion, kanban: renderKanban, ideas: renderIdeas, biblioteca: renderBiblioteca, clientes: renderClientes, metricas: renderMetricas, config: renderConfig };
...
function renderCalendarList(items){
  ...
  return `<div class="panel calendar-list-panel"><div class="panel-head"><div><div class="panel-title">Plan del mes en lista</div><div class="item-desc">Lectura rápida: fecha de salida para cliente, empresa, canales, tipo y estado.</div></div></div>
    <div class="plan-table"><div class="plan-row plan-head"><div>Publicación estimada</div><div>Título</div><div>Empresa</div><div>Plataforma</div><div>Tipo</div><div>Estado</div><div></div></div>
...
function renderOptionList(kind){
  ...
  return rows.length ? `<div class="list option-list">${rows.map(label=>{
    ...
    return `<div class="company-row"><div class="company-row-main"><div class="mini-logo">${kind==='channel'?'📡':'🏷️'}</div><div class="item-main"><div class="company-name">${esc(label)}</div><div class="company-sub">${isDefault?'Opción base':'Personalizado'}</div></div></div><div class="item-actions">${isDefault?'<button class="btn ghost sm" disabled>Base</button>':`<button class="btn danger sm" onclick="deleteOption('${row.id}')">Eliminar</button>`}</div></div>`;
  }).join('')}</div>` : '<div class="empty">Sin opciones</div>';
}
function openOptionModal(kind){
  const title = kind==='channel' ? 'Nuevo canal' : 'Nuevo tipo de contenido';
