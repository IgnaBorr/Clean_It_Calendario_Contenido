...
const APP_BUILD = 'final-109-calendar-fases-unicas-202605';
...
function renderCalendarPill(item, iso, kind='publish'){
  ...
  const title = kind === 'deadline'
    ? `Deadline interno: ${item.title}`
    : isPublished
      ? `Publicado: ${item.title}`
      : `Publicación estimada: ${item.title}`;
  const labelPrefix = kind === 'deadline' ? '⏱' : isPublished ? '✅' : '📣';
  const visualClass = kind === 'deadline' ? 'deadline' : isPublished ? 'status-publicado' : 'publish-estimated';
  const rangeMark = kind === 'publish' && pos === 'start' ? '▶ ' : kind === 'publish' && pos === 'mid' ? '━ ' : kind === 'publish' && pos === 'end' ? '◀ ' : '';
  return `<span class="cal-pill ${visualClass} range-${pos}" onclick="event.stopPropagation();openContentModal('${item.id}')" title="${escAttr(title)}">${labelPrefix} ${rangeMark}${esc(item.title)}</span>`;
}
...
function activeCalendarPhase(item, iso){
  if(!item || item.status === 'publicado') return null;
  const phaseByStatus = {
    idea: {label:'Idea', icon:'💡', cls:'phase-idea', start:item.idea_start_date, end:item.idea_end_date || item.idea_start_date},
    produccion: {label:'Producción', icon:'🎬', cls:'phase-prod', start:item.production_start_date, end:item.production_end_date || item.production_start_date},
    revision: {label:'Edición / revisión', icon:'✂️', cls:'phase-review', start:item.review_start_date, end:item.review_end_date || item.review_start_date},
  };
  const phase = phaseByStatus[item.status];
  if(!phase || !phase.start || !isOnRange(iso, phase.start, phase.end)) return null;
  return phase;
}
function renderPhasePill(item, iso){
  const p = activeCalendarPhase(item, iso);
  if(!p) return '';
  const title = `${p.label}: ${item.title}`;
  return `<span class="cal-pill ${p.cls}" onclick="event.stopPropagation();openContentModal('${item.id}')" title="${escAttr(title)}">${p.icon} ${esc(item.title)}</span>`;
}
...
function renderContentFlow(item){
  ...
  const idx = statusFlowIndex(status);
  ...
  const publishDate = item?.publish_date ? `<div class="publish-date-note ${isPublished?'is-published':''}"><i class="fa-solid ${isPublished?'fa-circle-check':'fa-bullhorn'}"></i> ${isPublished?'Publicado':'Publicación estimada'}: <strong>${fmtDate(item.publish_date)}</strong></div>` : `<div class="publish-date-note"><i class="fa-regular fa-calendar"></i> Publicación estimada: <strong>sin definir</strong></div>`;
  return `<div class="client-section"><div class="client-section-title">Estado del contenido</div>${publishDate}<div class="status-line ${isPublished?'completed':''}">${steps.map((s,i)=>{
    const done = isPublished || i < idx;
    const active = !isPublished && i === idx;
    return `<div class="status-step ${done?'done':active?'active':''}"><div class="status-dot">${done?'✓':s.icon}</div><div class="status-label">${s.label}</div></div>`;
  }).join('')}</div></div>`;
}
...
function renderCalendario(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    const phaseItems = items.filter(i=>activeCalendarPhase(i, iso)).sort((a,b)=>String(a.title).localeCompare(String(b.title)));
    const publishItems = items.filter(i=>isPublishedOnDate(i, iso)).sort((a,b)=>String(a.title).localeCompare(String(b.title)));
    const deadlineItems = items.filter(i=>i.internal_deadline===iso && !isPublishedOnDate(i, iso) && !activeCalendarPhase(i, iso));
    ...
    <div class="panel">
      <div class="panel-head">
        <div>
          ...
          <div class="item-desc">Tocá cualquier día para crear una pieza. Se muestra una única fase activa por contenido y la publicación estimada como hito separado.</div>
        </div>
      ...
      <div class="calendar-legend">
        ...
        <span class="legend-dot publish-estimated"></span> Publicación estimada
        <span class="legend-dot status-publicado"></span> Publicado
...
function renderCalendarList(items, ym){
  ...
  const list = items.filter(i=>itemInMonth(i, ym))
    ...
    .sort((a,b)=>String(a.publish_date || itemPrimaryDate(a)).localeCompare(String(b.publish_date || itemPrimaryDate(b))));
  return `<div class="panel client-list-panel">
    <div class="panel-head"><div><div class="panel-title">Lista del mes</div><div class="item-desc">Vista tipo plan para cliente: fecha estimada de publicación, plataforma, tipo, estado y comentarios.</div></div></div>
    ...
    <div class="content-table-wrap"><table class="content-table"><thead><tr><th>Publicación estimada</th><th>Hora</th><th>Título</th><th>Plataforma</th><th>Tipo</th><th>Estado</th><th>Cmt</th><th></th></tr></thead><tbody>
      ${list.map(i=>`<tr>
        <td>${i.publish_date ? fmtDate(i.publish_date) : '<span class="muted-cell">Sin definir</span>'}</td>
    ...
    <div class="form-grid">
      ...
      <div class="form-group span-2"><label class="form-label">Plan de fechas</label>
        <div class="date-plan-grid">
          ...
          <div class="date-plan-card single-date"><div><strong>Publicación estimada</strong><small>Fecha estimada de salida. Cuando salga, Ribera marca estado Publicado.</small></div><input class="form-input" type="date" name="publish_date" value="${escAttr(autoPublishDate)}" /></div>
        </div>
      </div>
