...
function renderPublicationCalendarEvent(item){
  ...
  const icon = published ? '✅' : platformIcon(item.platform || '');
  return `<div class="cal-pill publication-pill ${cls} ${platformClass(item.platform)} seg-single" draggable="true" ondragstart="dragPublicationEvent(event,'${item.id}')" onclick="event.stopPropagation();openPublicationModal('${item.id}')" title="${escAttr(item.content_type||'Publicación')} · ${escAttr(item.platform||'-')}"><span class="platform-logo-mini">${icon}</span><strong>${esc(item.title)}</strong><em>${esc(item.content_type||'')}</em>${companyLabel}</div>`;
}
function renderCalendarioPublicacion(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    const dayItems = items.filter(i=>i.publish_date===iso).sort((a,b)=>String(a.platform||'').localeCompare(String(b.platform||'')) || String(a.content_type||'').localeCompare(String(b.content_type||'')) || String(a.title||'').localeCompare(String(b.title||'')));
...
function renderPublicationList(items){
  ...
  const rows = items.filter(i=>String(i.publish_date||'').startsWith(month)).sort((a,b)=>String(a.publish_date).localeCompare(String(b.publish_date)) || String(a.platform||'').localeCompare(String(b.platform||'')) || String(a.title).localeCompare(String(b.title)));
...
function openPublicationModal(id=null, defaultDate=''){
  ...
  const body = `<form id="publication-form"><div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">Plataforma</label><select class="form-input" name="platform">${selectOptions(optionValues('channel'), item?.platform || '')}</select></div>
...
window.savePublicationItem = async(id='')=>{
  ...
  if(!payload.platform) return toast('Elegí una plataforma.', 'error');
