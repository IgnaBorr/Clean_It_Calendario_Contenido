...
function checklistArray(item){
  ...
  return [
    {done:false,text:'Video principal'},
    {done:false,text:'Pieza vertical adaptada'},
    {done:false,text:'Portada / thumbnail'},
    {done:false,text:'Copy / caption'},
    {done:false,text:'Subtítulos'},
    {done:false,text:'Stories o piezas derivadas'},
  ];
}
    ...
    <div class="form-grid">
      ...
      <div class="form-group span-2"><label class="form-label">Plan de fechas</label>
        <div class="date-plan-grid">
          <div class="date-plan-card single-date"><div><strong>Idea</strong><small>Fecha en la que se define o presenta la idea.</small></div><input class="form-input" type="date" name="idea_start_date" value="${escAttr(item?.idea_start_date||presetDate||'')}" /></div>
          <div class="date-plan-card range-date"><div><strong>Producción</strong><small>Rango real de grabación / armado.</small></div><input class="form-input" type="date" name="production_start_date" value="${escAttr(item?.production_start_date||'')}" /><span>→</span><input class="form-input" type="date" name="production_end_date" value="${escAttr(item?.production_end_date||item?.production_start_date||'')}" /></div>
          <div class="date-plan-card range-date"><div><strong>Edición / revisión</strong><small>Rango de edición, ajustes y revisión.</small></div><input class="form-input" type="date" name="review_start_date" value="${escAttr(item?.review_start_date||'')}" /><span>→</span><input class="form-input" type="date" name="review_end_date" value="${escAttr(item?.review_end_date||item?.review_start_date||'')}" /></div>
          <div class="date-plan-card single-date"><div><strong>Publicación</strong><small>Fecha final publicada o programada.</small></div><input class="form-input" type="date" name="publish_date" value="${escAttr(item?.publish_date||presetDate||'')}" /></div>
        </div>
      </div>
      ...
      <div class="form-group span-2"><label class="form-label">Elementos de producción / entregables previstos</label><div class="item-desc form-helper">No son etapas. Son piezas o elementos concretos a preparar: reel, historia, portada, copy, subtítulos, adaptación, etc.</div><div class="checklist-editor">${checklist.map((c,idx)=>`<div class="checklist-row"><input type="checkbox" class="checklist-done" ${c.done?'checked':''}><input class="form-input checklist-text" value="${escAttr(c.text||'')}" placeholder="Ej: Reel 30s, historia, portada, copy, subtítulos..."></div>`).join('')}<button type="button" class="btn ghost sm" onclick="addChecklistRow()">+ Agregar ítem</button></div></div>
...
window.addChecklistRow = ()=>{
  ...
  if(!box || !btn) return;
  btn.insertAdjacentHTML('beforebegin', `<div class="checklist-row"><input type="checkbox" class="checklist-done"><input class="form-input checklist-text" placeholder="Ej: Reel 30s, historia, portada, copy, subtítulos..."></div>`);
};
