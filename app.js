  ...
  return `<div class="lineas-page">
    <div class="panel lineas-hero">
      <div class="panel-head">
        ...
        <button class="btn primary sm" onclick="openContentLineModal(null,${jsArg(currentCompanyId)})"><i class="fa-solid fa-plus"></i> Nueva línea</button>
      </div>
...
function renderCompanyContentLines(company){
  ...
  return `<div class="panel company-lines-section" style="--company:${meta.color};--company-dark:${meta.dark};--company-soft:${meta.soft}">
    <div class="panel-head">
      ...
      <button class="btn primary sm" onclick="openContentLineModal(null,${jsArg(company.id)})"><i class="fa-solid fa-plus"></i> Agregar línea</button>
    </div>
  ...
  return `<div class="content-line-card ${line.synthetic?'synthetic':''}">
    <div class="content-line-head">
      ...
      <div class="item-actions">
        ${line.synthetic?`<button class="btn ghost sm" onclick="openContentLineModal(null,${jsArg(company.id)},${jsArg(line.label)})">Registrar</button>`:`<button class="btn ghost sm" onclick="openContentLineModal(${jsArg(line.id)})">Editar</button><button class="btn danger sm" onclick="deleteContentLine(${jsArg(line.id)})">Quitar</button>`}
      </div>
    </div>
    ...
    <div class="line-actions-row">
      <button class="btn ghost sm" onclick="openIdeaForLine(${jsArg(company.id)},${jsArg(line.label)})"><i class="fa-solid fa-lightbulb"></i> Idea</button>
      <button class="btn ghost sm" onclick="openContentForLine(${jsArg(company.id)},${jsArg(line.label)})"><i class="fa-solid fa-clapperboard"></i> Reel / producción</button>
      <button class="btn ghost sm" onclick="openPublicationForLine(${jsArg(company.id)},${jsArg(line.label)})"><i class="fa-solid fa-bullhorn"></i> Publicación</button>
    </div>
...
function renderAssociatedLineList(title, rows, companyId, isPublication=false){
  ...
  return `<div class="associated-line-section"><div class="associated-line-title">${esc(title)}</div>${max.map(i=>`<button class="associated-chip" onclick="${isPublication?`openPublicationModal('${i.id}')`:(i.status==='idea'?`openIdeaModal('${i.id}')`:`openContentModal('${i.id}')`)}"><span>${esc(i.title)}</span><em>${esc(isPublication ? (i.content_type||'Publicación') : statusLabel(i.status))}</em></button>`).join('')}${rows.length>5?`<div class="muted-mini">+${rows.length-5} más</div>`:''}</div>`;
}
...
window.deleteContentLine = async(id)=>{
  ...
  const msg = total ? `¿Quitar la línea "${line.label}"? No borra contenidos, pero desvincula ${total} pieza(s) de esta línea.` : `¿Quitar la línea "${line.label}"?`;
  if(!confirm(msg)) return;
  const { error: e1 } = await sb.from('content_items').update({content_line:null, updated_by:session.user.id}).eq('company_id', line.company_id).eq('content_line', line.label);
  if(e1) return throwError(e1);
  const { error: e2 } = await sb.from('publication_items').update({content_line:null, updated_by:session.user.id}).eq('company_id', line.company_id).eq('content_line', line.label);
  if(e2) return throwError(e2);
  const { error } = await sb.from('content_options').update({is_active:false}).eq('id', id);
  if(error) return throwError(error);
