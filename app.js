...
function renderCalendario(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    cells += `<div class="cal-day ${isToday?'today':''}" onclick="openContentModal(null,null,'${iso}')" title="Agregar contenido para ${fmtDate(iso)}">
      <div class="cal-day-top"><div class="cal-num">${d}</div><button class="cal-add-btn" onclick="event.stopPropagation();openContentModal(null,null,'${iso}')">+</button></div>
  ...
  return `${renderStats()}
    <div class="panel">
      <div class="panel-head">
        ...
        <div class="panel-actions"><button class="btn ghost sm" onclick="currentMonth--; if(currentMonth<0){currentMonth=11;currentYear--;} render();">← Mes anterior</button><strong>${MONTHS[currentMonth]} ${currentYear}</strong><button class="btn ghost sm" onclick="currentMonth++; if(currentMonth>11){currentMonth=0;currentYear++;} render();">Mes siguiente →</button></div>
      </div>
      <div class="calendar-legend">
        <span class="legend-dot status-idea"></span> Idea
        <span class="legend-dot status-produccion"></span> Producción
        <span class="legend-dot status-revision"></span> Edición / revisión
        <span class="legend-dot status-publicado"></span> Terminado
...
window.saveContent = async(id='')=>{
  ...
  ['internal_deadline','publish_date','publish_end_date'].forEach(k=>{ if(!payload[k]) payload[k] = null; });
