...
let state = { companies: [], items: [], assets: [], prospects: [], month_notes: [], responsibles: [], content_options: [] };
let isBusy = false;
...
function publicationCalendarItems(){
  return calendarScopeItems().filter(i=>i.status!=='idea' && i.publish_date);
}
function renderPublicationCalendarEvent(item){
  ...
  const icon = item.status==='publicado' ? '✅' : '📣';
  const cls = item.status==='publicado' ? 'phase-publicado' : 'phase-publicacion';
  const types = item.content_type || 'Contenido';
  const channels = item.channel || '-';
  return `<div class="cal-pill ${cls} seg-single" onclick="event.stopPropagation();openContentModal('${item.id}')" title="${escAttr(types)} · ${escAttr(channels)}">${icon} <strong>${esc(item.title)}</strong>${companyLabel}</div>`;
}
function renderCalendarioPublicacion(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    const dayItems = items.filter(i=>i.publish_date===iso).sort((a,b)=>String(a.content_type||'').localeCompare(String(b.content_type||'')) || String(a.title||'').localeCompare(String(b.title||'')));
    cells += `<div class="cal-day publication-day" data-date="${iso}" ondblclick="openContentModal(null,'publicacion_estimada','${iso}')">
      <div class="cal-num"><span>${d}</span><button class="cal-add" onclick="event.stopPropagation();openContentModal(null,'publicacion_estimada','${iso}')">+</button></div>
  ...
  return `<div class="panel">
    <div class="calendar-nav">
      ...
      <div><div class="calendar-title">Calendario de publicación · ${MONTHS[currentMonth]} ${currentYear}</div><div class="item-desc">Plan de salida para cliente: qué se busca subir cada día. Doble click o + para crear una pieza con publicación estimada.</div></div>
      <button class="btn ghost sm" onclick="changeMonth(1)">Siguiente →</button>
    </div>
    <div class="calendar-legend"><span class="legend-dot phase-publicacion">📣 Publicación estimada</span><span class="legend-dot phase-publicado">✅ Publicado</span></div>
...
function renderPublicationList(items){
  ...
  const rows = items.filter(i=>String(i.publish_date||'').startsWith(month)).sort((a,b)=>String(a.publish_date).localeCompare(String(b.publish_date)) || String(a.title).localeCompare(String(b.title)));
  return `<div class="panel calendar-list-panel"><div class="panel-head"><div><div class="panel-title">Plan de publicación en lista</div><div class="item-desc">Vista cliente: fecha estimada, pieza, plataforma, tipo y estado.</div></div></div>
    <div class="plan-table"><div class="plan-row plan-head"><div>Fecha estimada</div><div>Título</div><div>Empresa</div><div>Plataforma</div><div>Tipo</div><div>Estado</div><div></div></div>
      ${rows.length ? rows.map(i=>`<div class="plan-row"><div>${fmtDate(i.publish_date)}</div><div><strong>${esc(i.title)}</strong></div><div>${esc(companyNameById(i.company_id))}</div><div>${esc(i.channel || '-')}</div><div>${esc(i.content_type || '-')}</div><div><span class="pill ${statusPill(i.status)}">${statusLabel(i.status)}</span></div><div><button class="btn ghost sm" onclick="openContentModal('${i.id}')"><i class="fa-regular fa-eye"></i></button></div></div>`).join('') : `<div class="empty">Sin publicaciones estimadas para este mes.</div>`}
    </div></div>`;
}
...
async function loadAll(){
  ...
  const [companies, items, assets, monthNotes, responsibles, contentOptions, prospects] = await Promise.all(promises);
