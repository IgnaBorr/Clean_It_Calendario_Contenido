...
function renderPublicationCalendarEvent(item){
  ...
  return `<div class="cal-pill publication-pill ${cls} ${platformClass(item.platform)} seg-single" onclick="event.stopPropagation();openPublicationModal('${item.id}')" title="${escAttr(item.content_type||'Publicación')} · ${escAttr(item.platform||'-')}"><span class="platform-logo-mini">${icon}</span><strong>${esc(item.title)}</strong><em>${esc(item.content_type||'')}</em>${companyLabel}</div>`;
}
function renderCalendarioPublicacion(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    cells += `<div class="cal-day publication-day" data-date="${iso}" ondblclick="openPublicationModal(null,'${iso}')">
...
function openContentModal(id=null, status=null, defaultDate=''){
  ...
  </form>${item ? `<div class="content-detail-section"><div class="panel-title">Vista previa de entregables</div>${renderDeliverablePreviewList(item)}</div>` : ''}`;
...
function openIdeaModal(id=null){
  ...
  </div></form>${item ? `<div class="content-detail-section"><div class="panel-title">Vista previa de entregables</div>${renderDeliverablePreviewList(item)}</div>` : ''}`;
...
function openResponsibleModal(id=null){
  ...
  const body = `<form id="responsible-form"><div class="form-group"><label class="form-label">Nombre</label><input class="form-input" name="name" required value="${escAttr(r?.name||'')}" placeholder="Nacho, Lilu..." /></div></form>${item ? `<div class="content-detail-section"><div class="panel-title">Vista previa de entregables</div>${renderDeliverablePreviewList(item)}</div>` : ''}`;
