...
const APP_BUILD = 'final-101-hardfix-multicanal-fases-operativo-202605';
...
let sb = null;
let session = null;
let currentView = localStorage.getItem(`${STORAGE_PREFIX}_view`) || 'inicio';
let currentCompanyId = localStorage.getItem(`${STORAGE_PREFIX}_company`) || '';
let currentMonth = new Date().getMonth();
let currentYear = new Date().getFullYear();
let state = { companies: [], items: [], assets: [], prospects: [], profiles: [], access: [], contentAssetLinks: [], contentComments: [], schemaIssue: null };
let appReady = false;
let isBusy = false;
...
const uniqueClean = arr => [...new Set((arr || []).map(x=>String(x||'').trim()).filter(Boolean))];
function normalizeArrayField(value){
  ...
  if(typeof value === 'string'){
    ...
    try {
      ...
      if(Array.isArray(parsed)) return uniqueClean(parsed);
    } catch {}
    return uniqueClean(raw.split(','));
  }
  return [];
}
function itemChannels(item){ return normalizeArrayField(item?.channels).length ? normalizeArrayField(item.channels) : normalizeArrayField(item?.channel); }
function itemContentTypes(item){ return normalizeArrayField(item?.content_types).length ? normalizeArrayField(item.content_types) : normalizeArrayField(item?.content_type); }
function labelList(list, fallback='Sin dato'){
  const arr = uniqueClean(list);
  if(!arr.length) return fallback;
  return arr.length <= 2 ? arr.join(' + ') : `${arr.slice(0,2).join(' + ')} +${arr.length-2}`;
}
function checkboxGroup(name, options, selected=[]){
  const chosen = uniqueClean(selected);
  return `<div class="multi-choice">${options.map(opt=>`<label class="choice-chip"><input type="checkbox" name="${name}" value="${escAttr(opt)}" ${chosen.includes(opt)?'checked':''}> <span>${esc(opt)}</span></label>`).join('')}</div>`;
}
function getCheckedValues(name){ return $$(`input[name="${name}"]:checked`).map(x=>x.value); }
...
function contentLinks(itemId){ return state.contentAssetLinks.filter(l => l.content_item_id === itemId); }
function contentLinkedAssets(itemId){
  const links = contentLinks(itemId);
  return links.map(l => ({ link:l, asset: state.assets.find(a=>a.id===l.asset_id) })).filter(x=>x.asset);
}
function contentComments(itemId){ return state.contentComments.filter(c => c.content_item_id === itemId).sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at))); }
...
function checklistProgress(item){
  const list = parseChecklist(item?.checklist);
  const total = list.length;
  const done = list.filter(x=>!!x.done).length;
  return {done,total,label: total ? `${done}/${total}` : '0/0'};
}
function isImageAsset(asset){
  const url = asset?.link_url || '';
  const type = normalize(asset?.type || '');
  return type.includes('foto') || type.includes('imagen') || /\.(png|jpe?g|webp|gif|svg)(\?.*)?$/i.test(url);
}
function formatDateTime(v){
  if(!v) return '';
  try { return new Date(v).toLocaleString('es-AR', {day:'2-digit', month:'2-digit', hour:'2-digit', minute:'2-digit'}); } catch { return v; }
}
...
const PHASES = [
  {key:'idea', label:'Idea', start:'idea_start_date', end:'idea_end_date', icon:'💡'},
  {key:'produccion', label:'Producción', start:'production_start_date', end:'production_end_date', icon:'🎬'},
  {key:'revision', label:'Edición / revisión', start:'review_start_date', end:'review_end_date', icon:'👁️'}
];
function phaseStart(item, phase){ return item?.[phase.start] || ''; }
function phaseEnd(item, phase){ return item?.[phase.end] || item?.[phase.start] || ''; }
function isPhaseOnDate(item, iso, phase){
  const start = phaseStart(item, phase);
  if(!start) return false;
  const end = phaseEnd(item, phase) || start;
  return iso >= start && iso <= end;
}
function phaseRangePosition(item, iso, phase){
  const start = phaseStart(item, phase);
  const end = phaseEnd(item, phase) || start;
...
function currentPhaseLabel(item){
  const today = todayISO();
  const active = PHASES.find(ph=>isPhaseOnDate(item, today, ph));
  return active ? active.label : '';
}
function phaseHint(item){
  return PHASES.map(ph=>{
    const s = phaseStart(item, ph), e = phaseEnd(item, ph);
    return s ? `${ph.label}: ${fmtDate(s)}${e && e!==s ? ' → '+fmtDate(e) : ''}` : '';
  }).filter(Boolean).join(' · ');
}
...
function renderCalendarPill(item, iso, kind='publish'){
  ...
  const title = kind === 'deadline' ? `Deadline interno: ${item.title}` : `${item.title} · ${labelList(itemChannels(item),'Sin canal')}`;
  ...
  return `<span class="cal-pill ${kind==='deadline'?'deadline':`status-${item.status}`} range-${pos}" onclick="event.stopPropagation();openContentModal('${item.id}')" title="${escAttr(title)}">${labelPrefix} ${rangeMark}${esc(item.title)} <small>${esc(labelList(itemChannels(item),'Sin canal'))}</small></span>`;
}
function renderPhasePill(item, iso, phase){
  const pos = phaseRangePosition(item, iso, phase);
  const title = `${phase.label}: ${item.title}`;
  const rangeMark = pos === 'start' ? '▶ ' : pos === 'mid' ? '━ ' : pos === 'end' ? '◀ ' : '';
  return `<span class="cal-pill phase phase-${phase.key} range-${pos}" onclick="event.stopPropagation();openContentModal('${item.id}')" title="${escAttr(title)}">${phase.icon} ${rangeMark}${esc(item.title)}</span>`;
}
...
async function init(){
  ...
  sb.auth.onAuthStateChange((event, newSession)=>{
    session = newSession;
    if(!session){ appReady = false; showAuth(); return; }
    if(appReady){
...
async function bootApp(){
  ...
  if(currentCompanyId && availableCompanies.some(c=>c.id===currentCompanyId)){
    hide($('#company-gate')); show($('#app')); applyCompanyTheme(); render();
  } else {
    renderCompanyGate();
  }
}
...
async function loadAll(){
  ...
  const promises = [
    ...
    sb.from('content_comments').select('*').order('created_at', {ascending:false})
  ];
  if(hasRiberaAccess() || isRiberaUser()) promises.push(sb.from('prospects').select('*').order('updated_at', {ascending:false}));
  const [companies, items, assets, profiles, access, contentAssetLinks, contentComments, prospects] = await Promise.all(promises);
  ...
  if(contentAssetLinks.error) throwError(new Error('Falta ejecutar migration_final_operativo.sql o no hay permisos para content_asset_links.'));
  if(contentComments.error) throwError(new Error('Falta ejecutar migration_final_operativo.sql o no hay permisos para content_comments.'));
  ...
  state.contentAssetLinks = contentAssetLinks.data || [];
  state.contentComments = contentComments.data || [];
...
async function validateFinalSchema(){
  state.schemaIssue = null;
  if(!sb) return;
  const required = 'id,channels,content_types,idea_start_date,idea_end_date,production_start_date,production_end_date,review_start_date,review_end_date,checklist,resources_to_use';
  const { error } = await sb.from('content_items').select(required).limit(1);
  if(error){
    state.schemaIssue = 'Falta ejecutar migration_final_101_repair.sql o Supabase todavía tiene cache de schema viejo. Ejecutá la migración y recargá la web con ?v=final101.';
    console.error('Schema final incompleto:', error);
    toast(state.schemaIssue, 'error');
  }
}
...
function switchView(v){
  ...
  currentView = v;
  localStorage.setItem(`${STORAGE_PREFIX}_view`, currentView);
...
function render(){
  ...
  const viewHtml = (views[currentView] || renderInicio)();
  content.innerHTML = `${renderHubAlerts()}${viewHtml}`;
}


function renderHubAlerts(){
  if(!currentCompanyId || currentView === 'config') return '';
  const items = filteredItems();
  const overdue = items.filter(isOverdue);
  const dueToday = items.filter(isDueToday);
  const noDeadline = items.filter(i=>i.status!=='publicado' && !i.internal_deadline);
  const riberaDue = canViewClients() ? state.prospects.filter(p=>p.next_contact_date && p.next_contact_date <= todayISO() && !['Ganado','Perdido'].includes(p.status||'')) : [];
  const blocks = [];
  if(overdue.length) blocks.push(`<button class="alert-chip red" onclick="switchView('inicio')"><i class="fa-solid fa-triangle-exclamation"></i>${overdue.length} contenido${overdue.length===1?'':'s'} atrasado${overdue.length===1?'':'s'}</button>`);
  if(dueToday.length) blocks.push(`<button class="alert-chip amber" onclick="switchView('inicio')"><i class="fa-solid fa-clock"></i>${dueToday.length} vencen hoy</button>`);
  if(noDeadline.length) blocks.push(`<button class="alert-chip blue" onclick="switchView('kanban')"><i class="fa-solid fa-calendar-xmark"></i>${noDeadline.length} sin deadline interno</button>`);
  if(riberaDue.length) blocks.push(`<button class="alert-chip red" onclick="switchView('clientes')"><i class="fa-solid fa-phone"></i>${riberaDue.length} contacto${riberaDue.length===1?'':'s'} comercial${riberaDue.length===1?'':'es'} vencido${riberaDue.length===1?'':'s'}</button>`);
  return blocks.length ? `<div class="hub-alerts">${blocks.join('')}</div>` : '';
}
...
function renderRiberaDashboard(){
  const items = filteredItems();
  const activeProspects = state.prospects.filter(p=>!['Ganado','Perdido'].includes(p.status||''));
  const dueProspects = activeProspects.filter(p=>p.next_contact_date && p.next_contact_date <= todayISO());
  const estimatedPipeline = activeProspects.reduce((sum,p)=>sum + (Number(p.estimated_value)||0), 0);
  const productionActive = items.filter(i=>['produccion','revision'].includes(i.status));
  const validatedIdeas = items.filter(i=>i.status==='idea' && i.idea_stage==='validada');
  const overdue = items.filter(isOverdue);
  return `<div class="ribera-dashboard">
    <div class="ribera-hero panel">
      <div>
        <div class="panel-title">Dashboard interno Ribera</div>
        <h2>Producción, pipeline comercial y alertas en un solo lugar.</h2>
        <p class="item-desc">Esta vista es exclusiva del nivel Ribera. Sirve para ordenar la operación propia sin mezclarla con las cuentas cliente.</p>
      </div>
      <div class="ribera-hero-logo"><img src="${RIBERA_LOGO_PATH}" alt="Ribera" /></div>
    </div>
    <div class="stats-grid">
      ${stat('Pipeline activo', activeProspects.length, 'fa-handshake', 'bg-blue', `$${estimatedPipeline.toLocaleString('es-AR')} estimado`)}
      ${stat('Contactos vencidos', dueProspects.length, 'fa-phone', 'bg-red', 'seguimiento comercial')}
      ${stat('Producciones activas', productionActive.length, 'fa-clapperboard', 'bg-amber', 'producción + revisión')}
      ${stat('Ideas validadas', validatedIdeas.length, 'fa-lightbulb', 'bg-green', 'listas para producir')}
    </div>
    <div class="section-grid">
      <div>
        <div class="panel"><div class="panel-head"><div class="panel-title">Alertas operativas</div><button class="btn ghost sm" onclick="switchView('calendario')">Calendario</button></div>
          ${overdue.length ? `<div class="list">${overdue.slice(0,6).map(renderItem).join('')}</div>` : '<div class="empty">Sin contenidos atrasados.</div>'}
        </div>
        <div class="panel"><div class="panel-head"><div class="panel-title">Producciones activas</div><button class="btn ghost sm" onclick="switchView('kanban')">Kanban</button></div>
          ${productionActive.length ? `<div class="list">${productionActive.slice(0,8).map(renderItem).join('')}</div>` : '<div class="empty">No hay producciones internas activas.</div>'}
        </div>
      </div>
      <div>
        <div class="panel"><div class="panel-head"><div class="panel-title">Seguimiento comercial</div><button class="btn primary sm" onclick="switchView('clientes')">Ver pipeline</button></div>
          ${dueProspects.length ? `<div class="list">${dueProspects.slice(0,6).map(p=>`<div class="item"><div class="item-main"><div class="item-title">${esc(p.company_name)}</div><div class="item-meta"><span class="pill red">Contactar</span><span>${fmtDate(p.next_contact_date)}</span></div><div class="item-desc">${esc(p.next_action||'Sin próxima acción')}</div></div><div class="item-actions"><button class="btn ghost sm" onclick="openProspectModal('${p.id}')">Abrir</button></div></div>`).join('')}</div>` : '<div class="empty">Sin contactos comerciales vencidos.</div>'}
        </div>
        <div class="panel"><div class="panel-head"><div class="panel-title">Ideas listas para activar</div><button class="btn ghost sm" onclick="switchView('ideas')">Banco de ideas</button></div>
          ${validatedIdeas.length ? `<div class="list">${validatedIdeas.slice(0,6).map(renderItemCompact).join('')}</div>` : '<div class="empty">No hay ideas validadas pendientes.</div>'}
        </div>
      </div>
    </div>
  </div>`;
...
function renderInicio(){
  if(canViewClients()) return renderRiberaDashboard();
  const items = filteredItems();
  ...
  return `<div class="item">
    <div class="item-main">
      ...
      <div class="item-meta">
        ...
        <span>${esc(labelList(itemChannels(i),'Sin canal'))}</span>
        <span>Resp: ${esc(i.owner || 'Sin responsable')}</span>
        <span>Deadline: ${fmtDate(i.internal_deadline)}</span>
        <span>Checklist: ${checklistProgress(i).label}</span>
        <span>${contentLinks(i.id).length} asset${contentLinks(i.id).length===1?'':'s'}</span>
        <span>${contentComments(i.id).length} comentario${contentComments(i.id).length===1?'':'s'}</span>
      </div>
      ${i.next_action ? `<div class="item-desc"><strong>Próxima acción:</strong> ${esc(i.next_action)}</div>` : ''}${phaseHint(i)?`<div class="item-desc"><strong>Etapas:</strong> ${esc(phaseHint(i))}</div>`:''}
      ${i.resources_to_use ? `<div class="item-desc"><strong>Se va a usar:</strong> ${esc(i.resources_to_use)}</div>` : ''}
    </div>
...
function renderItemCompact(i){
  return `<div class="item">
    <div class="item-main">
      <div class="item-title">${esc(i.title)}</div>
      <div class="item-meta"><span class="pill ${statusPill(i.status)}">${statusLabel(i.status)}</span><span>${fmtDate(i.internal_deadline)}</span><span>Checklist ${checklistProgress(i).label}</span></div>
    </div>
...
function renderCalendario(){
  ...
  for(let d=1; d<=dim; d++){
    ...
    const phaseItems = PHASES.flatMap(ph => items.filter(i=>isPhaseOnDate(i, iso, ph)).map(i=>({item:i, phase:ph})));
    ...
    cells += `<div class="cal-day ${isToday?'today':''} ${canCreate?'can-create':''}" onclick="${canCreate?`openContentModal(null,null,'${iso}')`:''}" title="${canCreate?`Agregar contenido para ${fmtDate(iso)}`:'Solo lectura'}">
      ...
      ${phaseItems.map(x=>renderPhasePill(x.item, iso, x.phase)).join('')}
    ...
    <div class="panel">
      <div class="panel-head">
        <div>
          ...
          <div class="item-desc">Tocá cualquier día para crear una pieza con esa fecha precargada. Usá fecha inicio/fin cuando una fijación dure varios días.</div>
        </div>
      ...
      <div class="calendar-legend">
        ...
        <span class="legend-dot phase-produccion"></span> Tiempo de producción
        <span class="legend-dot phase-revision"></span> Tiempo de edición / revisión
        <span class="legend-dot status-publicado"></span> Fijación / terminado
...
function renderKanbanCard(i){
  return `<div class="kanban-card" onclick="openContentModal('${i.id}')">
    <div class="card-row"><span class="pill ${statusPill(i.status)}">${esc(labelList(itemChannels(i),'Canal'))}</span>${isOverdue(i)?'<span class="pill red">Atrasado</span>':''}</div>
    <div class="card-title">${esc(i.title)}</div>
    <div class="item-meta"><span>${fmtDate(i.internal_deadline)}</span><span>${esc(i.owner || 'Sin responsable')}</span><span>Checklist ${checklistProgress(i).label}</span><span>${contentLinks(i.id).length} assets</span></div>
...
window.generateLocalIdeas = async()=>{
  ...
  const rows = base.map(title=>({
    ...
    status:'idea', idea_stage:'cruda', priority:'media', channel:'Instagram', content_type:'Reel', channels:['Instagram'], content_types:['Reel'],
...
function renderConfig(){
  ...
  return `<div class="config-build-note">Build activo: ${esc(APP_BUILD)} · Usuario: ${esc(session?.user?.email || '')} · Admin: ${isAdminUser() ? 'sí' : 'no'}${state.schemaIssue ? ' · ⚠ ' + esc(state.schemaIssue) : ''}</div><div class="config-grid">
...
function renderChecklistEditor(item){
  const list = parseChecklist(item?.checklist);
  const rows = list.length ? list : [{text:'', done:false}];
  return `<div class="checklist-editor" id="checklist-editor">
    ${rows.map((row,idx)=>renderChecklistRow(row,idx)).join('')}
  </div>
  <button type="button" class="btn ghost sm" onclick="addChecklistRow()"><i class="fa-solid fa-plus"></i> Agregar ítem</button>`;
}
function renderChecklistRow(row={}, idx=0){
  return `<div class="check-row" data-check-row>
    <input type="checkbox" ${row.done?'checked':''} />
    <input class="form-input" value="${escAttr(row.text||'')}" placeholder="Ej: Buscar referencias / editar reel / revisar copy" />
    <button type="button" class="btn ghost sm" onclick="this.closest('[data-check-row]').remove()"><i class="fa-solid fa-xmark"></i></button>
  </div>`;
}
window.addChecklistRow = ()=>{
  const box = $('#checklist-editor');
  if(box) box.insertAdjacentHTML('beforeend', renderChecklistRow({}, Date.now()));
};
function collectChecklist(){
  return $$('#checklist-editor [data-check-row]').map(row=>({
    done: row.querySelector('input[type="checkbox"]')?.checked || false,
    text: row.querySelector('input.form-input')?.value.trim() || ''
  })).filter(x=>x.text);
}
function renderAssetAssociations(item, companyId){
  const links = item?.id ? contentLinks(item.id) : [];
  const selected = new Set(links.map(l=>l.asset_id));
  const assets = state.assets.filter(a=>a.company_id === companyId);
  if(!item?.id){
    return `<div class="content-side-panel"><div class="panel-title">Assets asociados</div><div class="empty">Guardá el contenido una vez para asociar assets de biblioteca.</div></div>`;
  }
  return `<div class="content-side-panel">
    <div class="panel-title">Assets asociados</div>
    ${assets.length ? `<div class="asset-association-list">${assets.map(a=>`<label class="asset-association-card ${selected.has(a.id)?'selected':''}">
      <input type="checkbox" class="content-asset-check" value="${a.id}" ${selected.has(a.id)?'checked':''} />
      <div class="asset-preview-small">${isImageAsset(a) && a.link_url ? `<img src="${escAttr(a.link_url)}" alt="${escAttr(a.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><span style="display:none"><i class="fa-solid ${assetIcon(a.type)}"></i></span>` : `<span><i class="fa-solid ${assetIcon(a.type)}"></i></span>`}</div>
      <div><strong>${esc(a.name)}</strong><small>${esc(a.type||'Asset')}</small></div>
    </label>`).join('')}</div>` : '<div class="empty">No hay assets en la biblioteca de esta empresa.</div>'}
  </div>`;
}
function renderLinkedAssets(item){
  if(!item?.id) return '';
  const linked = contentLinkedAssets(item.id);
  if(!linked.length) return `<div class="empty">Este contenido todavía no tiene assets asociados.</div>`;
  return `<div class="linked-assets-grid">${linked.map(({asset})=>`<div class="linked-asset-card">
    <div class="linked-asset-preview">${isImageAsset(asset) && asset.link_url ? `<img src="${escAttr(asset.link_url)}" alt="${escAttr(asset.name)}" onerror="this.style.display='none';this.nextElementSibling.style.display='flex'" /><span style="display:none"><i class="fa-solid ${assetIcon(asset.type)}"></i></span>` : `<span><i class="fa-solid ${assetIcon(asset.type)}"></i></span>`}</div>
    <div class="item-title">${esc(asset.name)}</div>
    <div class="item-meta"><span>${esc(asset.type||'Asset')}</span></div>
    ${asset.link_url ? `<a class="btn ghost sm full" target="_blank" href="${escAttr(asset.link_url)}">Abrir</a>` : ''}
  </div>`).join('')}</div>`;
}
async function syncContentAssets(contentId){
  const selectedIds = $$('.content-asset-check').filter(x=>x.checked).map(x=>x.value);
  const { error: delError } = await sb.from('content_asset_links').delete().eq('content_item_id', contentId);
  if(delError) return throwError(delError);
  if(selectedIds.length){
    const rows = selectedIds.map(assetId=>({content_item_id:contentId, asset_id:assetId, role:'referencia', created_by:session.user.id}));
    const { error: insError } = await sb.from('content_asset_links').insert(rows);
    if(insError) return throwError(insError);
  }
}
function renderContentComments(item){
  if(!item?.id) return `<div class="content-comments"><div class="panel-title">Comentarios</div><div class="empty">Guardá el contenido para habilitar comentarios.</div></div>`;
  const comments = contentComments(item.id);
  return `<div class="content-comments">
    <div class="panel-title">Comentarios</div>
    <div class="comment-list">${comments.length ? comments.map(c=>`<div class="comment-box"><div class="comment-head"><strong>${esc(c.created_by_email||'Equipo')}</strong><span>${formatDateTime(c.created_at)}</span></div><div>${esc(c.body)}</div>${canEditCompanyData(item.company_id)?`<button class="comment-delete" onclick="deleteContentComment('${c.id}','${item.id}')">Eliminar</button>`:''}</div>`).join('') : '<div class="empty">Sin comentarios.</div>'}</div>
    ${canEditCompanyData(item.company_id)?`<div class="comment-compose"><textarea class="form-input" id="new-comment-body" placeholder="Comentario interno: cambios, referencias, ajustes, aprobación..."></textarea><button class="btn primary sm" onclick="addContentComment('${item.id}')">Comentar</button></div>`:''}
  </div>`;
}
window.addContentComment = async(contentId)=>{
  const body = $('#new-comment-body')?.value.trim();
  if(!body){ toast('Escribí un comentario.', 'error'); return; }
  const { error } = await sb.from('content_comments').insert({content_item_id:contentId, body, created_by:session.user.id, created_by_email:session.user.email});
  if(error) return throwError(error);
  await loadAll(); openContentModal(contentId); toast('Comentario agregado');
};
window.deleteContentComment = async(commentId, contentId)=>{
  if(!confirm('¿Eliminar este comentario?')) return;
  const { error } = await sb.from('content_comments').delete().eq('id', commentId);
  if(error) return throwError(error);
  await loadAll(); openContentModal(contentId); toast('Comentario eliminado');
};
function openContentModal(id=null, status=null, presetDate=null){
  const item = id ? state.items.find(i=>i.id===id) : null;
  const targetCompanyId = item?.company_id || currentCompanyId;
  const canEdit = !item ? canEditCompanyData(targetCompanyId) : canEditCompanyData(item.company_id);
  const body = `<form id="content-form">
    <div class="form-grid">
      <div class="form-group"><label class="form-label">Empresa</label><select class="form-input" name="company_id" ${item?'disabled':''}>${companyOptions(targetCompanyId)}</select>${item?`<input type="hidden" name="company_id" value="${item.company_id}" />`:''}</div>
      ...
      <div class="form-group"><label class="form-label">Fijación inicio</label><input class="form-input" type="date" name="publish_date" value="${escAttr(item?.publish_date||presetDate||'')}" /></div>
      <div class="form-group"><label class="form-label">Fijación fin</label><input class="form-input" type="date" name="publish_end_date" value="${escAttr(item?.publish_end_date||item?.publish_date||presetDate||'')}" /></div>
      <div class="form-group span-2"><label class="form-label">Canales</label>${checkboxGroup('channels', CHANNELS, itemChannels(item))}<div class="field-hint">Podés seleccionar varios: por ejemplo Instagram + Reels + TikTok.</div></div>
      <div class="form-group span-2"><label class="form-label">Tipos de contenido</label>${checkboxGroup('content_types', CONTENT_TYPES, itemContentTypes(item))}<div class="field-hint">Una misma tarea puede ser Reel + Historia + Post, sin duplicar producción.</div></div>
      <div class="form-group span-2"><label class="form-label">Tiempos de trabajo por etapa</label>
        <div class="phase-grid">
          <div><span>Idea</span><input class="form-input" type="date" name="idea_start_date" value="${escAttr(item?.idea_start_date||presetDate||'')}" /><input class="form-input" type="date" name="idea_end_date" value="${escAttr(item?.idea_end_date||item?.idea_start_date||'')}" /></div>
          <div><span>Producción</span><input class="form-input" type="date" name="production_start_date" value="${escAttr(item?.production_start_date||'')}" /><input class="form-input" type="date" name="production_end_date" value="${escAttr(item?.production_end_date||item?.production_start_date||'')}" /></div>
          <div><span>Edición / revisión</span><input class="form-input" type="date" name="review_start_date" value="${escAttr(item?.review_start_date||'')}" /><input class="form-input" type="date" name="review_end_date" value="${escAttr(item?.review_end_date||item?.review_start_date||'')}" /></div>
        </div>
        <div class="field-hint">Estos rangos se ven en el calendario como barras de trabajo, independientes de la fijación/publicación.</div>
      </div>
      ...
      <div class="form-group span-2"><label class="form-label">Qué se va a usar</label><textarea class="form-input" name="resources_to_use" placeholder="Recursos previstos: foto de referencia, música, locación, toma, copy, producto, carpeta Drive...">${esc(item?.resources_to_use||'')}</textarea></div>
      <div class="form-group span-2"><label class="form-label">Checklist de producción</label>${renderChecklistEditor(item)}</div>
  ...
  <div class="content-detail-grid">
    <div><div class="panel-title detail-title">Assets visibles al abrir el contenido</div>${renderLinkedAssets(item)}</div>
    <div>${renderAssetAssociations(item, targetCompanyId)}</div>
  </div>
...
window.saveContent = async(id='')=>{
  if(state.schemaIssue){ toast(state.schemaIssue, 'error'); return; }
  const form = new FormData($('#content-form'));
  const payload = Object.fromEntries(form.entries());
  payload.tags = parseTags(payload.tags);
  payload.checklist = collectChecklist();
  payload.channels = getCheckedValues('channels');
  payload.content_types = getCheckedValues('content_types');
  payload.channel = payload.channels[0] || '';
  payload.content_type = payload.content_types[0] || '';
  ...
  if(payload.publish_date && !payload.publish_end_date) payload.publish_end_date = payload.publish_date;
  PHASES.forEach(ph=>{ if(payload[ph.start] && !payload[ph.end]) payload[ph.end] = payload[ph.start]; });
  for(const ph of PHASES){ if(payload[ph.start] && payload[ph.end] && payload[ph.end] < payload[ph.start]){ toast(`La fecha fin de ${ph.label} no puede ser anterior al inicio.`, 'error'); return; } }
  if(payload.publish_date && payload.publish_end_date && payload.publish_end_date < payload.publish_date){ toast('La fecha fin no puede ser anterior a la fecha de inicio.', 'error'); return; }
  ...
  if(id){
    ...
    const { data, error } = await sb.from('content_items').insert(payload).select('id').single();
    if(error) return throwError(error);
    savedId = data.id;
  }
  if(savedId) await syncContentAssets(savedId);
...
window.exportBackup = ()=>{
  const data = JSON.stringify({ version:1, exported_at:new Date().toISOString(), companies:state.companies, content_items:state.items, assets:state.assets, prospects:state.prospects, content_asset_links:state.contentAssetLinks, content_comments:state.contentComments, user_profiles:state.profiles, user_company_access:state.access }, null, 2);
