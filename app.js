...
function multiPlatformStyle(platforms=[]){
  ...
  const parts = colors.map((c,idx)=>{
    const start = (idx * 100 / colors.length).toFixed(2);
    const end = ((idx + 1) * 100 / colors.length).toFixed(2);
    return `${c} ${start}%, ${c} ${end}%`;
  }).join(', ');
  return `style="--pf1:${colors[0]};--multi-bg:linear-gradient(90deg, ${parts});"`;
}
...
function subscribeChanges(){
  ...
  channel = sb.channel('content-hub-db')
    ...
    .on('postgres_changes', {event:'*', schema:'public', table:'content_options'}, debounceReload)
    .on('postgres_changes', {event:'*', schema:'public', table:'publication_items'}, debounceReload)
    .subscribe();
}
