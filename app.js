...
const APP_BUILD = 'cliente-plan-final-202605';
...
function checklistArray(item){
  ...
  return [
    {done:false,text:'Idea definida'},
    {done:false,text:'Material / referencias listas'},
    {done:false,text:'Producción realizada'},
    {done:false,text:'Edición terminada'},
    {done:false,text:'Revisión interna'},
    {done:false,text:'Aprobación cliente'},
    {done:false,text:'Publicado / link final cargado'},
  ];
}
...
function renderApprovalMini(item){
  ...
  const inReview = item?.status === 'revision' || done;
  const inProd = ['produccion','revision','publicado'].includes(item?.status);
  const steps = [
    {label:'Edición', ok:inReview || done},
    {label:'Contenido', ok:inProd || inReview || done},
    {label:'Cliente', ok:done},
  ];
  return `<div class="approval-line">${steps.map(s=>`<div class="approval-node ${s.ok?'ok':''}"><span>${s.ok?'✓':'•'}</span><small>${s.label}</small></div>`).join('')}</div>`;
}
...
function renderCalendarList(items, ym){
  ...
  const list = items.filter(i=>itemInMonth(i, ym))
    ...
    .sort((a,b)=>String(a.publish_date || a.internal_deadline || a.idea_start_date || '').localeCompare(String(b.publish_date || b.internal_deadline || b.idea_start_date || '')));
  return `<div class="panel client-list-panel">
    ...
    <div class="content-table-wrap"><table class="content-table"><thead><tr><th>Fecha</th><th>Hora</th><th>Título</th><th>Plataforma</th><th>Tipo</th><th>Estado</th><th>Cmt</th><th></th></tr></thead><tbody>
      ${list.map(i=>`<tr>
        <td>${fmtDate(i.publish_date || i.internal_deadline || i.idea_start_date)}</td>
...
function openContentModal(id=null, status=null, presetDate=null){
  ...
  const checklist = checklistArray(item);
  const body = `<div class="content-client-view">
    ${item ? renderContentFlow(item) + renderApprovalMini(item) + renderDeliverablesPanel(item) + renderAssociatedAssetsPanel(item) + renderCommentsPanel(item) : ''}
  </div>
  <form id="content-form">
    <div class="form-grid">
      ...
      <div class="form-group"><label class="form-label">Tiempo de idea · inicio</label><input class="form-input" type="date" name="idea_start_date" value="${escAttr(item?.idea_start_date||presetDate||'')}" /></div>
      <div class="form-group"><label class="form-label">Tiempo de idea · fin</label><input class="form-input" type="date" name="idea_end_date" value="${escAttr(item?.idea_end_date||item?.idea_start_date||presetDate||'')}" /></div>
      <div class="form-group"><label class="form-label">Producción · inicio</label><input class="form-input" type="date" name="production_start_date" value="${escAttr(item?.production_start_date||'')}" /></div>
      <div class="form-group"><label class="form-label">Producción · fin</label><input class="form-input" type="date" name="production_end_date" value="${escAttr(item?.production_end_date||item?.production_start_date||'')}" /></div>
      <div class="form-group"><label class="form-label">Edición / revisión · inicio</label><input class="form-input" type="date" name="review_start_date" value="${escAttr(item?.review_start_date||'')}" /></div>
      <div class="form-group"><label class="form-label">Edición / revisión · fin</label><input class="form-input" type="date" name="review_end_date" value="${escAttr(item?.review_end_date||item?.review_start_date||'')}" /></div>

      <div class="form-group"><label class="form-label">Fijación / publicación inicio</label><input class="form-input" type="date" name="publish_date" value="${escAttr(item?.publish_date||presetDate||'')}" /></div>
      <div class="form-group"><label class="form-label">Fijación / publicación fin</label><input class="form-input" type="date" name="publish_end_date" value="${escAttr(item?.publish_end_date||item?.publish_date||presetDate||'')}" /></div>
      ...
      <div class="form-group span-2"><label class="form-label">Checklist de producción</label><div class="checklist-editor">${checklist.map((c,idx)=>`<div class="checklist-row"><input type="checkbox" class="checklist-done" ${c.done?'checked':''}><input class="form-input checklist-text" value="${escAttr(c.text||'')}" placeholder="Tarea del checklist"></div>`).join('')}<button type="button" class="btn ghost sm" onclick="addChecklistRow()">+ Agregar ítem</button></div></div>
...
window.addChecklistRow = ()=>{
  const box = document.querySelector('.checklist-editor');
  const btn = box?.querySelector('button');
  if(!box || !btn) return;
  btn.insertAdjacentHTML('beforebegin', `<div class="checklist-row"><input type="checkbox" class="checklist-done"><input class="form-input checklist-text" placeholder="Tarea del checklist"></div>`);
};
function readChecklist(){
  return $$('.checklist-row').map(row=>({ done: !!row.querySelector('.checklist-done')?.checked, text: row.querySelector('.checklist-text')?.value.trim() || '' })).filter(x=>x.text);
}
window.saveContent = async(id='')=>{
  ...
  payload.checklist = readChecklist();
  ...
  for(const pair of [['publish_date','publish_end_date'],['idea_start_date','idea_end_date'],['production_start_date','production_end_date'],['review_start_date','review_end_date']]){
    const [s,e]=pair;
