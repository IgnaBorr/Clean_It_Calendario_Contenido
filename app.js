...
function viewTitle(v){
  return ({inicio:'Inicio',calendario:'Calendario producción',calendario_publicacion:'Calendario publicación',ideas:'Ideas',biblioteca:'Biblioteca',clientes:'Posibles clientes',metricas:'Métricas',config:'Configuración'})[v] || 'Content Hub';
}
...
function optionValues(kind){
  const fromDb = state.content_options
    .filter(o=>o.kind===kind && (o.company_id===currentCompanyId || !o.company_id))
...
function bindStaticEvents(){
  ...
  $('#new-content-btn').addEventListener('click', ()=>{ currentView==='calendario_publicacion' ? openPublicationModal() : openContentModal(); });
...
function render(){
  ...
  const views = { inicio: renderInicio, calendario: renderCalendario, calendario_publicacion: renderCalendarioPublicacion, ideas: renderIdeas, biblioteca: renderBiblioteca, clientes: renderClientes, metricas: renderMetricas, config: renderConfig };
...
function renderConfig(){
  ...
  return `<div class="config-grid">
    ...
    <div class="panel"><div class="panel-head"><div><div class="panel-title">Campañas / líneas de contenido</div><div class="item-desc">Categorías estratégicas para ordenar ideas, producción y publicaciones.</div></div><button class="btn primary sm" onclick="openOptionModal('content_line')"><i class="fa-solid fa-plus"></i> Agregar</button></div>
      ${renderOptionList('content_line')}
    </div>
...
function renderOptionList(kind){
  ...
  const dbRows = state.content_options.filter(o=>o.kind===kind && (o.company_id===currentCompanyId || !o.company_id));
...
function selectOptions(values, selected=''){
  return [''].concat(values||[]).map(v=>`<option value="${escAttr(v)}" ${v===selected?'selected':''}>${esc(v || 'Seleccionar')}</option>`).join('');
}
...
function openPublicationModal(id=null, defaultDate=''){
  const item = id ? state.publication_items.find(x=>x.id===id) : null;
  const body = `<form id="publication-form"><div class="form-grid">
    <div class="form-group"><label class="form-label">Empresa / cliente</label><select class="form-input" name="company_id">${companyOptions(item?.company_id || currentCompanyId)}</select></div>
    ...
    <div class="form-group"><label class="form-label">Campaña / línea de contenido</label><select class="form-input" name="content_line">${selectOptions(optionValues('content_line'), item?.content_line || '')}</select></div>
    ...
    <div class="form-group"><label class="form-label">Tipo</label><select class="form-input" name="content_type">${selectOptions(optionValues('content_type'), item?.content_type || '')}</select></div>
...
function openContentModal(id=null, status=null, defaultDate=''){
  ...
  const body = `<form id="content-form">
    <div class="form-grid">
      ...
      ${multiCheckDropdown('content_type','Tipos de contenido', item?.content_type || '', optionValues('content_type'), 'Podés seleccionar varios.')}
...
function openIdeaModal(id=null){
  ...
  const body = `<form id="idea-form"><div class="form-grid">
    ...
    <div class="form-group"><label class="form-label">Campaña / línea</label><select class="form-input" name="content_line">${selectOptions(optionValues('content_line'), item?.content_line || '')}</select></div>
