...
const STORAGE_PREFIX = 'cleanit_supabase_content_hub';
...
let state = { companies: [], items: [], assets: [] };
let isBusy = false;
...
function viewTitle(v){
  return ({inicio:'Inicio',calendario:'Calendario',kanban:'Kanban',ideas:'Ideas',biblioteca:'Biblioteca',metricas:'Métricas',config:'Configuración'})[v] || 'Content Hub';
}
...
async function bootApp(){
  ...
  if(!currentCompanyId || !state.companies.some(c=>c.id===currentCompanyId)) currentCompanyId = state.companies[0]?.id || '';
...
async function loadAll(){
  setBusy(true);
  const [companies, items, assets] = await Promise.all([
  ...
  $('#company-gate').innerHTML = `<div class="company-entry">
    <div class="company-entry-card">
      <div class="company-entry-head">
        <div>
          <div class="brand-row"><div class="brand-mark">CI</div><div><div class="brand-title">Content Hub</div><div class="brand-subtitle">Supabase operativo</div></div></div>
          <h1>Elegí la empresa antes de trabajar</h1>
          <p>Los contenidos, ideas y assets se cargan en una base compartida. Lo que haga una persona lo ve la otra.</p>
        </div>
        <button class="btn ghost sm" onclick="window.__signOut()"><i class="fa-solid fa-arrow-right-from-bracket"></i> Salir</button>
      </div>
      <div class="company-grid">
        ${state.companies.map(c=>renderCompanyCard(c)).join('')}
      </div>
      ${!state.companies.length ? '<div class="empty">No hay empresas cargadas.</div>' : ''}
    </div>
  </div>`;
}
...
function render(){
  ...
  const views = { inicio: renderInicio, calendario: renderCalendario, kanban: renderKanban, ideas: renderIdeas, biblioteca: renderBiblioteca, metricas: renderMetricas, config: renderConfig };
...
function renderMetricas(){
  const perCompany = state.companies.map(c=>{
...
function renderConfig(){
  return `<div class="config-grid">
    <div class="panel"><div class="panel-head"><div><div class="panel-title">Empresas / clientes</div><div class="item-desc">Agregar, editar o eliminar empresas. Si borrás una con datos, se reasignan antes.</div></div><button class="btn primary sm" onclick="openCompanyModal()"><i class="fa-solid fa-plus"></i> Agregar</button></div>
      ${state.companies.map(c=>renderCompanyRow(c)).join('') || '<div class="empty">Sin empresas</div>'}
    </div>
...
function renderCompanyRow(c){
  ...
  return `<div class="company-row">
    ...
    <div class="item-actions"><button class="btn ghost sm" onclick="openCompanyModal('${c.id}')">Editar</button><button class="btn danger sm" onclick="deleteCompany('${c.id}')">Eliminar</button></div>
  </div>`;
}
...
function companyOptions(selected=currentCompanyId){
  return state.companies.map(c=>`<option value="${c.id}" ${c.id===selected?'selected':''}>${esc(c.name)}</option>`).join('');
}
...
window.deleteCompany = async(id)=>{
  ...
  if(!company) return;
  if(state.companies.length <= 1){ toast('Debe quedar al menos una empresa.', 'error'); return; }
  ...
  const fallback = state.companies.find(c=>c.id!==id);
...
window.exportBackup = ()=>{
  const data = JSON.stringify({ version:1, exported_at:new Date().toISOString(), companies:state.companies, content_items:state.items, assets:state.assets }, null, 2);
